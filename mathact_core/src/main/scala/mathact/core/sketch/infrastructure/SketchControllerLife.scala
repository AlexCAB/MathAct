...
private [mathact] trait SketchControllerLife { _: SketchControllerActor ⇒
  ...
  def startSketchController(): Unit = {
    ...
    sketchUi ! M.SetSketchUIStatusString("DriveCreating...", Color.Black)}
  ...
  def sketchBuiltTimeout(state: SketchController.State): Unit = state match{
    case SketchController.State.Building ⇒
      log.error(
