  ...
  trait TestCase extends Suite{
    ...
    class TestBlockWithUI extends TestBlock with BlockUILike{
      ...
      private[core] def uiShow(): Unit = {
        showFrameUICalled = true
        waitOrFail()}
      private[core] def uiHide(): Unit = {
        hideFrameCalled = true
        waitOrFail()}
...
//      builtBlockWithUi.isHideFrameCalled shouldEqual true}
    "not execute uiShow() and uiHide() twice" in new BlocksWithUi {
      ...
      //TODO Проверить чтобы при повтороной посалке ShowBlockUi или HideBlockUi импелер получал только одно событие в очредь.
...
//      testBlockWithUi.getLastUiEvent shouldEqual Some(e1)}
    ...
    //TODO Проверить чтобы при быстрой постановке в очередь отправитель замедлялся (отслежывать по текущемцу системному времени).
